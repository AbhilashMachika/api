...
app.config['MAIL_USERNAME'] = '317ae6ff4801ea'
app.config['MAIL_PASSWORD'] = 'fb579489747a91'
...
def super_simple():
    return jsonify(message='Hello from the Planetary API. happy')
...
@jwt_required
def update_planet():
    planet_id = int(request.form.get('planet_id', False))
    planet = Planet.query.filter_by(planet_id=planet_id).first()
    if planet:
